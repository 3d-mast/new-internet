...
function render() {
  ...
  $("#versionLine").textContent = `${status.product || "Reef"} ${status.release || "4.0.0"} · ${status.protocol || "LICHEN/2"}`;
  ...
  let text = autopilot.reason || "Reef контролирует маршруты автоматически.";
  let badge = "ожидание";
  let stateClass = "";
  if (!enabled) {
    title = "Сеть остановлена";
    text = "Нажмите одну кнопку — Reef сам проверит узлы и построит лучший маршрут.";
    badge = "выключено";
    ...
    text = "Создайте приглашение или вставьте полученный ключ. Остальное Reef настроит сам.";
    badge = "нет узлов";
  } else {
  ...
  $("#statusText").textContent = `${cfg.node_name || "Reef"} · ${shortID(status.node_id)}`;
...
$("#joinButton").addEventListener("click", async () => {
  try {
    ...
    $("#joinDialog").close();
    $("#joinToken").value = "";
    toast("Устройство подключено. Reef строит маршрут.");
    await refresh();
  } catch (error) { toast(error.message, true); }
});
